...
func (r *Raft) handleAppendEntries(m pb.Message) {
	...
	if m.Index >= r.RaftLog.FirstIndex() {
		...
		// 如果该节点的 prevLogTerm 与该节点 entries 在 prevLogIndex 处 term
		// 不相同，则找到该 term 所在 entries 中的最小 index，并返回给 leader 该
		// term 与 index
	...
	if r.RaftLog.committed < m.Commit {
		...
		// A: if m.Entries is empty, it won't enter the loop, maybe the commitIndex
...
func (r *Raft) handleAppendEntriesResponse(m pb.Message) {
	...
	if m.Reject {
		...
		if m.LogTerm != None {
			logTerm := m.LogTerm
			// leader 找到第一个大于 follower 所提供的 term 的 log index
			sliceIndex := sort.Search(len(r.RaftLog.entries),
				func(i int) bool { return r.RaftLog.entries[i].Term > logTerm })
			// 如果在该 index 前一个的 log term 还是不相同，那么需要重复上述
			// 过程直至找到 leader 与 该 follower 日志相同的位置
			// 日志相同的位置：index 与 term 均相同
